...
from tkinter import Tk, Toplevel, Event, TclError, StringVar, Frame, Menu, Label, Entry, SOLID, RIDGE, \
    DISABLED, NORMAL, N, S, E, W, LEFT, messagebox, Canvas
...
class Nse:
    ...
    def __init__(self, window: Tk) -> None:
        ...
        self.output_columns: Tuple[str, str, str, str, str, str, str, str, str] = (
            'Time', f'LTP_Call_{self.sp1}',f'profit\nCall_{self.sp1}',
            f'LTP_Put_{self.sp1}', f'profit\nPut_{self.sp1}',
            f'LTP_Call_{self.sp2}', f'profit\nCall_{self.sp2}',
            f'LTP_Put_{self.sp3}', f'profit\nPut_{self.sp3}', 'Net\nProfit')
    ...
    def login_win(self, window: Tk) -> None:
        ...
        r+=1
        date_label1: Label = Label(self.login, text="Expiry Date for Strike Price1: ", justify=LEFT)
        ...
        r+=1
        val_label1: Label = Label(self.login, text="BUY/SELL VAL for Strike Price1 CE: ")
        ...
        r+=1
        val_label2: Label = Label(self.login, text="BUY/SELL VAL for Strike Price1 PE: ")
        ...
        r+=1
        date_label2: Label = Label(self.login, text="Expiry Date for Strike Price2", justify=LEFT)
        ...
        r+=1
        val_label3: Label = Label(self.login, text="BUY/SELL VAL for STRIKE Price2 CE: ")
        ...
        r+=1
        val_label4: Label = Label(self.login, text="BUY/SELL VAL for STRIKE PRICE 3 PE: ")
    ...
    def start(self, event: Optional[Event] = None) -> None:
        ...
        try:
            ...
            if self.option_mode=='Index':
                ...
                self.output_columns: Tuple[str, str, str, str, str, str, str, str, str] = (
                    'Time', f'LTP_Put_{self.sp1}', f'profit\nPut_{self.sp1}',
                    f'LTP_Call_{self.sp2}', f'profit\nCall_{self.sp2}', 'Net\nProfit')
                self.csv_headers = self.output_columns   
    ...
    def links(self, link: str, event: Optional[Event] = None) -> None:
        ...
        if link == "developer":
            ...
            webbrowser.open_new("https://github.com/QuickLearner171998/Python-NSE-Option-Chain-Analyzer/blob/master/README.md/")
        elif link == "license":
            webbrowser.open_new("https://github.com/QuickLearner171998/Python-NSE-Option-Chain-Analyzer/blob/master/LICENSE/")
        elif link == "sources":
            webbrowser.open_new("https://github.com/QuickLearner171998/Python-NSE-Option-Chain-Analyzer/")
    ...
    def about(self, event: Optional[Event] = None) -> None:
        ...
        dev_val: Label = Label(self.info, text="Pramay Singhvi", fg="blue", cursor="hand2", relief=RIDGE)
    ...
    def close_main(self, event: Optional[Event] = None) -> None:
        ask_quit: bool = messagebox.askyesno("Quit", "Proceed to quit?", icon='warning',
                                             default='no')
        if ask_quit:
            self.session.close()
            if self.logging:
    ...
    def set_values(self) -> None:
        ...
        if self.option_mode=='Index':
            ...
            output_values: List[Union[str, float]] = [self.str_current_time, self.pe_ltp_1, self.pe_1_profit,  self.ce_ltp_2, self.ce_2_profit, self.net_profit]            
    ...
    def main(self) -> None:
        ...
        self.pe_1_profit = round((self.pe_val_1 - self.pe_ltp_1)*self.quantity, 3)
        self.ce_2_profit = round((self.ce_ltp_2 - self.ce_val_2)*self.quantity, 3)

        if self.option_mode=='Index':    
            self.ce_1_profit = round((self.ce_val_1 - self.ce_ltp_1)*self.quantity, 3)
            self.pe_3_profit = round((self.pe_ltp_3 - self.pe_val_3)*self.quantity, 3)
